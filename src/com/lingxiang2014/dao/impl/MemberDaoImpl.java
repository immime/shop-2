...
public class MemberDaoImpl extends BaseDaoImpl<Member, Long> implements MemberDao {
	...
	public Member findLeaf(Integer index) {
		Member leaf = null;
		if (index == null) {
			index = 0;
		}
		...
		List<Member> list = super.findList(criteriaQuery, null, null, null, orders);
	...
	public Member findLeaf(Member member, Integer count, Zone zone) {
		Member left = member.getLeftChildren();// 左边的人
		Member right = member.getRightChildren();// 右边的人
		Member leaf = null;
	...
	public Long count(MemberRank memberRank) {
		return find(memberRank) == null ? 0L : find(memberRank).size() + 0L;
	}

	public List<Member> find(MemberRank memberRank) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Member> criteriaQuery = criteriaBuilder.createQuery(Member.class);
		Root<Member> root = criteriaQuery.from(Member.class);
		criteriaQuery.select(root);
	...
	public Member find(Member parent, Integer index, MemberRank memberRank) {
		...
		order.add(Order.asc("number"));
		criteriaQuery.where(restrictions);
		List<Member> list = super.findList(criteriaQuery, null, null, null, order);
		if (list == null) {
			return null;
		} else if (list.size() <= index) {
