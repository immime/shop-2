...
public class TransferController extends BaseController {
	...
	public @ResponseBody
	Map<String, Object> checkBalance(String type) {
		Map<String, Object> data = new HashMap<String, Object>();
		Member member = memberService.getCurrent();
		if ((Type.xianjin.name()).equals(type)) {// 转到对方现金币账户
			data.put("balance", member.getBalance());
		} else if ((Type.baodan.name()).equals(type)) {// 转到对方报单币账户
			data.put("balance", member.getBalance1());
		} else if ((Type.futou.name()).equals(type)) {// 转到对方复投币账户
			data.put("balance", member.getBalance2());
		} else if ((Type.licai.name()).equals(type)) {// 转到对方理财币账户
			data.put("balance", member.getBalance3());

		} else if ((Type.xjTbd.name()).equals(type)) {// 现金币->报单币
			data.put("balance", member.getBalance());
		} else if ((Type.xjTft.name()).equals(type)) {// 现金币->复投币
			data.put("balance", member.getBalance());
		} else if ((Type.xjTlc.name()).equals(type)) {// 现金币->理财币
			data.put("balance", member.getBalance());
		} else if ((Type.bdTft.name()).equals(type)) {// 报单币->复投币
			data.put("balance", member.getBalance1());
		} else if ((Type.bdTlc.name()).equals(type)) {// 报单币->理财币
			data.put("balance", member.getBalance1());
		} else if ((Type.bfTli.name()).equals(type)) {// 复投币->理财币
			data.put("balance", member.getBalance2());
		} else if ((Type.bdTxj.name()).equals(type)) {// 报单币->现金币
			data.put("balance", member.getBalance1());
		} else if ((Type.ftTxj.name()).equals(type)) {// 复投币->现金币
			data.put("balance", member.getBalance2());
		} else if ((Type.lcTxj.name()).equals(type)) {// 理财币->现金币
			data.put("balance", member.getBalance3());
		} else if ((Type.ftTbd.name()).equals(type)) {// 复投币->报单币
			data.put("balance", member.getBalance2());
		} else if ((Type.lcTbd.name()).equals(type)) {// 理财币->报单币
			data.put("balance", member.getBalance3());
		} else if ((Type.lcTft.name()).equals(type)) {// 理财币->复投币
			data.put("balance", member.getBalance3());
		}
	...
	public @ResponseBody
	Map<String, Object> checkToMember(String toMemberUsername) {
		Map<String, Object> data = new HashMap<String, Object>();
	...
	public @ResponseBody
	boolean checkUsername(String toMemberName) {
	...
	public String save(Transfer transfer, String toMemberName, BigDecimal balance, HttpServletRequest request, HttpServletResponse response, ModelMap model, RedirectAttributes redirectAttributes) {
		...
		if (Type.xianjin.equals(transfer.getType())) {// 转到对方现金币账户
		...
		} else if (Type.baodan.equals(transfer.getType())) {// 转到对方报单币账户
		...
		} else if (Type.futou.equals(transfer.getType())) {// 转到对方复投币账户
		...
		} else if (Type.licai.equals(transfer.getType())) {// 转到对方理财币账户
	...
	public String save1(Transfer transfer, BigDecimal balance, HttpServletRequest request, HttpServletResponse response, ModelMap model, RedirectAttributes redirectAttributes) {
		...
		transfer.setBalance(balance);// 获取转账的金额
		if (Type.xjTbd.equals(transfer.getType())) {// 现金币->报单币
			transfer.setOldFromBalance(member.getBalance());
			transfer.setOldToBalance(member.getBalance1());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
			argument_A = "现金币->报单币";
			argument_B = "现金币";
			argument_C = "报单币";
		...
		} else if (Type.xjTft.equals(transfer.getType())) {// 现金币->复投币
			transfer.setOldFromBalance(member.getBalance());
		...
		} else if (Type.xjTlc.equals(transfer.getType())) {// 现金币->理财币
			transfer.setOldFromBalance(member.getBalance());
		...
		} else if (Type.bdTft.equals(transfer.getType())) {// 报单币->复投币
			transfer.setOldFromBalance(member.getBalance1());
			transfer.setOldToBalance(member.getBalance2());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
			...
			transfer.setNewFromBalance(member.getBalance1());
			transfer.setNewToBalance(member.getBalance2());
		} else if (Type.bdTlc.equals(transfer.getType())) {// 报单币->理财币
			transfer.setOldFromBalance(member.getBalance1());
			transfer.setOldToBalance(member.getBalance3());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
			...
			transfer.setNewFromBalance(member.getBalance1());
			transfer.setNewToBalance(member.getBalance3());
		} else if (Type.bfTli.equals(transfer.getType())) {// 复投币->理财币
			transfer.setOldFromBalance(member.getBalance2());
			transfer.setOldToBalance(member.getBalance3());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
			...
			transfer.setNewFromBalance(member.getBalance2());
			transfer.setNewToBalance(member.getBalance3());
		} else if (Type.bdTxj.equals(transfer.getType())) {// 报单币->现金币
			transfer.setOldFromBalance(member.getBalance1());
		...
		} else if (Type.ftTxj.equals(transfer.getType())) {// 复投币->现金币
			transfer.setOldFromBalance(member.getBalance2());
			transfer.setOldToBalance(member.getBalance());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
			...
			transfer.setNewFromBalance(member.getBalance2());
			transfer.setNewToBalance(member.getBalance());
		} else if (Type.lcTxj.equals(transfer.getType())) {// 理财币->现金币
			transfer.setOldFromBalance(member.getBalance3());
			transfer.setOldToBalance(member.getBalance1());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
			...
			transfer.setNewFromBalance(member.getBalance3());
			transfer.setNewToBalance(member.getBalance());
		} else if (Type.ftTbd.equals(transfer.getType())) {// 复投币->报单币
			transfer.setOldFromBalance(member.getBalance2());
			transfer.setOldToBalance(member.getBalance1());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
			...
			transfer.setNewFromBalance(member.getBalance2());
			transfer.setNewToBalance(member.getBalance1());
		} else if (Type.lcTbd.equals(transfer.getType())) {// 理财币->报单币
			transfer.setOldFromBalance(member.getBalance3());
			transfer.setOldToBalance(member.getBalance1());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
			...
			transfer.setNewFromBalance(member.getBalance3());
			transfer.setNewToBalance(member.getBalance1());
		} else if (Type.lcTft.equals(transfer.getType())) {// 理财币->复投币
			transfer.setOldFromBalance(member.getBalance3());
			transfer.setOldToBalance(member.getBalance2());
			transfer.setFee(transfer.getBalance().multiply(setting.getXjTbdRate()));// 手续费
			transfer.setBalance1(transfer.getBalance().subtract(transfer.getFee()));// 实到金额
