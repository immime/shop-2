...
public class MemberController extends BaseController {
	...
	Message signIn() {
		...
		Integer signInType = setting.getSignInType();// 获取签到金额发放的方式

		if (setting.getIsSignIn()) {
			List<Bonuds> list = bonudsService.findToday(Type.signInBonuds, member);// 获取今天签到记录

			if (list == null || list.size() == 0) {// 今天没有签到
				if (signInType == 1) {// 1:按金额领取
					signInMoney = setting.getSignInMoney();
				} else if (signInType == 2) {// 2:按比率领取
					signInMoney = member.getMemberRank().getAmount().multiply(setting.getSignInMoneyRate());
				} else if (signInType == 3) {// 3：随机领取
	...
	public @ResponseBody
	Map<String, Object> checkMemberRank(Long memberRankId) {
		Member member = memberService.getCurrent();
	...
	public String confirmActive(Long memberRankId, Long memberId, HttpServletRequest request, RedirectAttributes redirectAttributes) {
		...
		money = member1.getMemberRank().getAmount().subtract(money);// 计算多少金额的业绩
	...
	private void changePosition(Member member) {

		Member top = member.getTop();
		if (top != null && top.getMemberRank().getIsDefault()) {// 节点人没有激活
	...
	public BigDecimal devideDuiPengBonuds(Member member1) {
		...
		BigDecimal leftRemainResults = new BigDecimal(0);// 左边剩余业绩
		BigDecimal rightRemainResults = new BigDecimal(0);// 右边剩余业绩
		BigDecimal todayDuiPengBonuds = new BigDecimal(0);// 今天已经获得的平衡奖

		Setting setting = SettingUtils.get();
		BigDecimal maxTodayDuiPengBonuds = setting.getTodayDuiPengBonuds();// 获取系统设置的每天最高平衡奖金额
		BigDecimal remainTodayDuiPengBonuds = new BigDecimal(0);// 获取今天剩余可获得平衡奖
		...
		if (parent != null && parent.getIsDuiPengBonuds()) {
			leftRemainResults = parent.getLeftRemainResults();// 获取左边剩余业绩
			rightRemainResults = parent.getRightRemainResults();// 获取右边剩余业绩
			todayDuiPengBonuds = parent.getTodayDuiPengBonuds();// 获取今天已经获得的平衡奖
			maxTodayDuiPengBonuds = setting.getTodayDuiPengBonuds();// 获取系统设置的每天最高平衡奖金额
			remainTodayDuiPengBonuds = new BigDecimal(0);// 获取今天剩余可获得平衡奖

			remainTodayDuiPengBonuds = maxTodayDuiPengBonuds.subtract(todayDuiPengBonuds);

			String memo = "";
			BigDecimal money = new BigDecimal(0);// 计算可获得的平衡奖
			if (leftRemainResults.compareTo(rightRemainResults) > 0 && leftRemainResults.compareTo(zero) > 0 && rightRemainResults.compareTo(zero) > 0) {// 右边业绩是小区
			...
			} else if (leftRemainResults.compareTo(zero) > 0 && rightRemainResults.compareTo(zero) > 0) {// 左边业绩是小区
	...
	public void devideLeaderBonuds(Member member1, BigDecimal duiPengBonuds) {
		...
		if (parent != null && parent.getIsLeaderBonuds() && !parent.getMemberRank().getIsDefault()) {
			if (parent.getMyPeople() > 0) {// 至少推荐一个
		...
		parent = parent.getParent();
		if (parent != null) {
			if (parent.getMyPeople() >= 2 && parent.getIsLeaderBonuds()) {// 至少推荐2个
				money = duiPengBonuds.multiply(setting.getLeaderBonuds());
				String memo = "会员：" + member1.getNumber() + ",激活：" + member1.getMemberRank().getName() + "身份，领取" + setting.getLeaderBonuds().multiply(percent) + "%的领导奖。金额：" + money.setScale(setting.getPriceScale(), BigDecimal.ROUND_HALF_UP);
				// 把平衡奖的金额添加到现金币账户中
				memo = memo + "其中" + setting.getaRate().multiply(setting.getPercent()) + "%进入现金币账户。进入之前现金币账户余额：" + parent.getBalance().setScale(setting.getPriceScale(), BigDecimal.ROUND_HALF_UP) + ",";
		...
		if (parent != null && !parent.getMemberRank().getIsDefault()) {
			if (parent.getMyPeople() >= 3 && parent.getIsLeaderBonuds()) {// 至少直推3个
				money = duiPengBonuds.multiply(setting.getLeaderBonuds());
				String memo = "会员：" + member1.getNumber() + ",激活：" + member1.getMemberRank().getName() + "身份，领取" + setting.getLeaderBonuds().multiply(percent) + "%的领导奖。金额：" + money.setScale(setting.getPriceScale(), BigDecimal.ROUND_HALF_UP);
				// 把平衡奖的金额添加到现金币账户中
				memo = memo + "其中" + setting.getaRate().multiply(setting.getPercent()) + "%进入现金币账户。进入之前现金币账户余额：" + parent.getBalance().setScale(setting.getPriceScale(), BigDecimal.ROUND_HALF_UP) + ",";
		...
		parent = parent.getParent();
		if (parent != null && !parent.getMemberRank().getIsDefault()) {
			if (parent.getMyPeople() >= 4 && parent.getIsLeaderBonuds()) {// 直推4个或者多余4个
				money = duiPengBonuds.multiply(setting.getLeaderBonuds());
				String memo = "会员：" + member1.getNumber() + ",激活：" + member1.getMemberRank().getName() + "身份，领取" + setting.getLeaderBonuds().multiply(percent) + "%的领导奖。金额：" + money.setScale(setting.getPriceScale(), BigDecimal.ROUND_HALF_UP);
				// 把平衡奖的金额添加到现金币账户中
				memo = memo + "其中" + setting.getaRate().multiply(setting.getPercent()) + "%进入现金币账户。进入之前现金币账户余额：" + parent.getBalance().setScale(setting.getPriceScale(), BigDecimal.ROUND_HALF_UP) + ",";
	...
	public void updateResults(Member member, BigDecimal money) {
		...
		if (top != null) {
			...
			} else if (top.getRightChildren() != null && top.getRightChildren() == member) {// 放在右边
				// 增加右边的业绩
	...
	Message delete(Long id) {
		...
		if (leftMember == null && rightMember == null) {
			...
			if (leftMember != null && rightMember == null) {
				if (leftMember.getMemberRank().getIsDefault()) {// 左边会员没有激活
				...
				if (rightMember.getMemberRank().getIsDefault()) {// 右边会员没有激活
				...
				if (leftMember.getMemberRank().getIsDefault() && rightMember.getMemberRank().getIsDefault()) {// 左右会员没有激活
					delete(leftMember.getId());
					delete(rightMember.getId());
				} else if (!leftMember.getMemberRank().getIsDefault() && rightMember.getMemberRank().getIsDefault()) {// 左边会员已经激活
					leftMember.setTop(member.getTop());
					memberService.update(leftMember);
				} else if (leftMember.getMemberRank().getIsDefault() && !rightMember.getMemberRank().getIsDefault()) {// 右边会员已经激活
					rightMember.setTop(member.getTop());
					memberService.update(rightMember);
				} else {// 两边会员都激活了
					if (leftMember.getActiveDate().compareTo(rightMember.getActiveDate()) >= 0) {// 右边先激活的
						rightMember.setTop(member.getTop());
						memberService.update(rightMember);
